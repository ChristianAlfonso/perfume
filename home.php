 <!-- Header-->
 <header class="py-5 d-flex justify-content-center align-items-center" id="hero" style="background: url(./images/hero-bg.webp) no-repeat center / cover; height: 50vh;">
    <div class="container  m-3 p-5 rounded-3">
        <div class="text-center text-white">
            <h1 class="display-4 fw-bolder">MJCHUA PERFUME STORE</h1>
            <p class="lead mb-0">Looking for your signature scent? Discover our exclusive collection of perfumes crafted to match your unique style! Shop Now!</p>
        </div>
    </div>
</header>
<!-- Section-->
<section>
    <div class="container-fluid p-4 px-lg-5 bg-light">
        <div class="row row-cols-md-3 row-cols-xl-4 justify-content-center">
            <?php 
                $products = $conn->query("SELECT * FROM `products` where status = 1 order by rand() limit 8 ");
                while($row = $products->fetch_assoc()):
                    $upload_path = base_app.'/uploads/product_'.$row['id'];
                    $img = "";
                    if(is_dir($upload_path)){
                        $fileO = scandir($upload_path);
                        if(isset($fileO[2]))
                            $img = "uploads/product_".$row['id']."/".$fileO[2];
                        // var_dump($fileO);
                    }
                    $inventory = $conn->query("SELECT * FROM inventory where product_id = ".$row['id']);
                    $inv = array();
                    while($ir = $inventory->fetch_assoc()){
                        $inv[$ir['size']] = number_format($ir['price']);
                    }
            ?>
            <div class="col mb-5">
                <div class="card h-100 product-item">
                    <!-- Product image-->
                    <img class="card-img-top w-100" src="<?php echo validate_image($img) ?>" alt="..." />
                    <!-- Product details-->
                    <div class="card-body p-4">
                        <div class="text-center">
                            <!-- Product name-->
                            <h5 class="fw-bolder"><?php echo $row['product_name'] ?></h5>
                            <!-- Product price-->
                            <?php foreach($inv as $k=> $v): ?>
                                <span><b><?php echo $k ?>: </b><?php echo $v ?></span>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <!-- Product actions-->
                    <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                        <div class="text-center">
                            <a class="btn btn-flat btn-primary "   href=".?p=view_product&id=<?php echo md5($row['id']) ?>">View</a>
                        </div>
                        
                    </div>
                </div>
            </div>
            <?php endwhile; ?>
        </div>
    </div>
</section>

<script src="https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/gsap.min.js"></script>
<script>
 document.addEventListener("DOMContentLoaded", (event) => {

    
    gsap.from('#hero', {
        duration: 1.5,
        delay: 0.8,
        opacity: 0,
    });

 });

    gsap.from('.product-item', {
        duration: 1.5,
        y: 100,
        delay: 2,
        opacity: 0,
        stagger: 0.2,
    });
</script>